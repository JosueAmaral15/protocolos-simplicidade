...
- ❌ Apenas máquina local (não escala pela rede)
- ❌ Gerenciamento manual de painéis
- ❌ Daemon adiciona complexidade

**Quando Usar (Padrão):**
- Desenvolvimento local em Linux/macOS
- 2-4 IAs trabalhando concorrentemente
- Necessita feedback visual e supervisão
- Quer controle tátil sobre coordenação
- Trabalhando em ambiente pesado de terminal (seu estilo preferido)

---

#### 🔍 Protocolo de Detecção e Seleção de Opção

**OBRIGATÓRIO**: Antes de escolher uma opção de coordenação, a IA DEVE detectar se outra IA já estabeleceu coordenação.

**Passos de Detecção:**

```bash
# 1. Verificar Opção A (arquivo JSON compartilhado)
if [ -f /tmp/ai_coordination.json ]; then
    echo "Opção A detectada - usando sistema de arquivos compartilhado"
    # Usar Opção A
fi

# 2. Verificar Opção B (orquestrador)
if curl -s http://127.0.0.1:5555/get_context?agent_id=probe >/dev/null 2>&1; then
    echo "Opção B detectada - orquestrador executando na porta 5555"
    # Usar Opção B
fi

# 3. Verificar Opção C (tmux + daemon)
if [ -f /tmp/tmux_ai_state.json ] && tmux has-session -t ai-coord 2>/dev/null; then
    echo "Opção C detectada - sessão tmux 'ai-coord' ativa"
    # Usar Opção C
fi

# 4. Se nenhuma detectada - perguntar ao usuário
if [ nenhuma coordenação detectada ]; then
    echo "Nenhum sistema de coordenação detectado."
    echo "Múltiplas IAs trabalhando neste projeto?"
    read -p "Escolha: (C) tmux [padrão], (B) orquestrador, (A) filesystem: " choice
    # Configurar opção escolhida
fi
```

**Hierarquia de Fallback (se configuração falhar):**

```
Tentar C (tmux) → se falhar → Tentar B (orquestrador) → se falhar → Usar A (filesystem)
```

**Exemplos de Cenários de Falha:**
- **C falha**: tmux não instalado → recorrer a B
- **B falha**: Não pode vincular porta 5555, sem Python → recorrer a A
- **A sempre funciona**: Sistema de arquivos sempre disponível (último recurso)

---

#### ✅ Checklist de Coordenação Multi-IA

**Antes de Iniciar Trabalho Multi-IA:**
- [ ] Detectei se coordenação já existe?
- [ ] Se detectada → estou usando a MESMA opção que outras IAs?
- [ ] Se NÃO detectada → perguntei ao usuário qual opção usar?
- [ ] A infraestrutura de coordenação está executando (daemon/orquestrador)?
- [ ] Registrei meu agente com ID e papel?
- [ ] Entendo a hierarquia de fallback (C → B → A)?

**Durante o Trabalho:**
- [ ] Estou verificando estado compartilhado antes de ações principais?
- [ ] Estou atualizando meu status após decisões?
- [ ] Estou respeitando bloqueios de arquivo de outras IAs?
- [ ] Estou evitando arquivos sendo editados atualmente?
- [ ] Estou enviando mensagens para coordenar (não trabalhando cego)?

**Prevenção de Conflitos:**
- [ ] Antes de editar arquivo → verificar se bloqueado por outra IA
- [ ] Antes de operações git → verificar se outra IA está no meio de commit
- [ ] Antes de executar testes → verificar se outra IA modificou arquivos de teste
- [ ] Se conflito detectado → pedir orientação ao usuário

**Após Completar Tarefa:**
- [ ] Atualizei status para "completed"?
- [ ] Liberei todos os bloqueios de arquivo?
- [ ] Notifiquei outras IAs da conclusão?
- [ ] Documentei minhas mudanças no estado compartilhado?

---

#### 🎓 Regras de Ouro Multi-IA

1. **Nunca assuma isolamento**: Sempre verifique se outras IAs estão trabalhando
2. **Coordenação é obrigatória**: Se múltiplas IAs detectadas → DEVE usar uma das 3 opções
3. **Mesma opção para todas**: Todas as IAs devem usar o mesmo método de coordenação
4. **Fallback é aceitável**: Se opção preferida falhar, use a próxima na hierarquia
5. **Usuário decide conflitos**: Se IAs discordarem sobre propriedade de arquivo → perguntar ao usuário
6. **Feedback visual**: Opção C (tmux) é padrão porque visibilidade previne erros
7. **Produção usa B**: Para trabalho empresarial/crítico, Opção B (orquestrador) é mais robusta
8. **Opção A é último recurso**: Use coordenação de filesystem apenas se B e C falharem
